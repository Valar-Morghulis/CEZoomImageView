...
float DefaultZoomStep = 2; //放大倍数
int DefaultZoomCount = 1;//放大次数
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        self._imageView = [[[CTDownloadImageView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)] autorelease];
        self._imageView._contentMode = UIViewContentModeScaleAspectFill;
        
        [self._imageView set_defaultImage:[UIImage imageNamed:DEFAULT_EMPTY_IMAGE]];
...
-(void)set_zoomCount:(int)zoomCount
{
